...
static pax_col_t palette[] = {0xffffffff, 0xff000000, 0xffff0000};  // (flipped white/black?) white, black, red
#endif
...
#if defined(CONFIG_BSP_TARGET_KAMI)
#define BLACK 1 // swapped from 'white'?
#define WHITE 0 // swapped from 'black'?
#define RED   2
#else
...
#define RED   0xFFFF0000
    ...
    // Not implimentated kami?
    bsp_led_set_brightness(20);

	    // LEDs [dimmed illumination level], kami goes from 0 (right) to 5 (left)
	    bsp_led_set_pixel(5, 0x2A0000);  // Red
...
//    vTaskDelay(pdMS_TO_TICKS(500));
    ...
    pax_background(&fb, WHITE);
    pax_draw_text(&fb, BLACK, pax_font_sky_mono, 16, 0, 0, "HACK HACK HACK HACK HACK\nTEST TEST TEST TEST TEST\n\n   NUBcore t'was here!\n\nHACK HACK HACK HACK HACK\nFUCK SHIT DAMN CRAP FRAQ");
    blit();
	...
	bsp_input_event_t event;
	    ...
	    // LEDs [dimmed illumination level], kami goes from 0 (right) to 5 (left)
	    bsp_led_set_pixel(5, 0x0C0000);  // Red
	    bsp_led_set_pixel(4, 0x000C00);  // Green
	    bsp_led_set_pixel(3, 0x00000C);  // Blue
	    bsp_led_set_pixel(2, 0x0C0C00);  // Yellow
	    bsp_led_set_pixel(1, 0x000C0C);  // Magenta
	    bsp_led_set_pixel(0, 0x0C000C);  // Cyan
	    bsp_led_send();                  // Send data to the coprocessor

	    vTaskDelay(pdMS_TO_TICKS(200));

	    // LEDs [dimmed illumination level], kami goes from right to left
	    bsp_led_set_pixel(0, 0x0C0000);  // Red
	    bsp_led_set_pixel(1, 0x000C00);  // Green
	    bsp_led_set_pixel(2, 0x00000C);  // Blue
	    bsp_led_set_pixel(3, 0x0C0C00);  // Yellow
	    bsp_led_set_pixel(4, 0x000C0C);  // Magenta
	    bsp_led_set_pixel(5, 0x0C000C);  // Cyan
	    bsp_led_send();                  // Send data to the coprocessor

//        if (xQueueReceive(input_event_queue, &event, portMAX_DELAY) == pdTRUE) {
        if (xQueueReceive(input_event_queue, &event, pdMS_TO_TICKS(800)) == pdTRUE) {
